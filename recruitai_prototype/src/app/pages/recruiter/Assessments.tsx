...
const assessedCandidates = candidates.filter(
  c => c.stage === 'Ranked' || c.stage === 'Interview Done'
);
...
export default function Assessments() {
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="mb-6">
        <h1 style={{ fontSize: '1.375rem', fontWeight: 600, color: '#E2E4EB' }}>Assessments</h1>
        <p className="text-sm mt-0.5" style={{ color: '#7E8494' }}>
          {assessedCandidates.length} submitted coding assessments
        </p>
      </div>

      <div className="space-y-3">
        {assessedCandidates.map(c => (
          <div
            key={c.id}
            ...
            onClick={() => navigate(`/recruiter/candidates/${c.id}/assessment`)}
            onMouseEnter={e => {
          ...
          >
            ...
            <div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-5 flex-wrap">
              ...
              <div className="flex items-center gap-2 text-xs">
                <span className="px-2 py-0.5 rounded" style={{ backgroundColor: '#0D1017', color: '#7DA4D4', fontFamily: 'monospace' }}>
                  Python
                </span>
                <span style={{ color: '#7E8494' }}>Two Sum</span>
              </div>
              <div className="flex items-center gap-1.5">
                <Code2 size={13} style={{ color: '#7C6AEF' }} />

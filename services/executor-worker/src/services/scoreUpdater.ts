...
export async function persistExecutionResults(
	...
	try {
		const { data: application } = await supabaseAdmin
			.from('applications')
			.select('job_id, cv_score, voice_score, system_design_score')
			.eq('id', input.applicationId)
			.maybeSingle()
		...
		if (application) {
			...
			if (job) {
				...
				if (allPresent) {
					console.info(`[executor-worker] All stage scores present for application ${input.applicationId}, triggering aggregation via scoring API`)
					...
					await fetch(scoringUrl, {
						...
						console.warn(`[executor-worker] Failed to trigger scoring API: ${err instanceof Error ? err.message : err}`)
					})
				}
			}
		}
	} catch (err) {
		console.warn(`[executor-worker] Auto-scoring check failed: ${err instanceof Error ? err.message : err}`)
	}
